...
≼≽ *Link:* ${vid.url}
❖────────────────❖`
  conn.sendButton(m.chat, `❖────━•[YOUTUBE]•━────❖
≼≽ *Judul:* ${title}
≼≽ *Durasi:* ${durationH}
≼≽ *Views:* ${viewH}
≼≽ *Upload:* ${publishedTime}
≼≽ *Link:* ${vid.url}
❖────────────────❖`, author.trim(), await( await conn.getFile(thumbnail)).data, ['📽VIDEO', `${usedPrefix}ytv ${url} 360`], false, { quoted: m, 'document': { 'url':'https://github.com/denz-aja' },
'mimetype': global.dpdf,
'fileName': `YOUTUBE PLAYS`,
...
body: wm,
sourceUrl: 'https://github.com/denz-aja', thumbnail: await ( await conn.getFile(thumbnail)).data
  }
 } 
})
